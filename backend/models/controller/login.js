    ...
    try {
        ...
        if (adminLoginStatus) {
            ...
            if (isValidUser) {
                ...
                res.status(200).json({ message: "admin authentication failed" })
            }
        }
        else {
            ...
            if (userLoginStatus) {
                ...
                if (isValidUser) {
                    ...
                    res.status(200).json({ message: "user authentication failed" })
                }
            } else {

                res.status(200).json({ message: 'details not found' })
            }
        }
    } catch (err) {
        console.error(err);
